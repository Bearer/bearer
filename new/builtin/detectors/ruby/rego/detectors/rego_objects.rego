...
detections_at contains node := detections if {
	results := curio.query.match_at(hash_pair_query, node)
	len(results) != 0

  properties := [property |
    property := curio.evaluator.for_node(results[_].pair], "rego_properties")[_]
  ]
...
detections_at contains node := detections if {
	result := curio.query.match_once_at(parent_pair_query, node)
  result

	objects := curio.evaluator.for_node(result.value, "rego_objects")
