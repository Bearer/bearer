...
namespace
{
  ...
  struct Scanner
  {
    ...
    bool scan_template_text(TSLexer *lexer)
    {
      if (!tags.size())
        return false;
...
extern "C"
{
  ...
  void *tree_sitter_html2_external_scanner_create()
  {
    return new Scanner();
  }

  bool tree_sitter_html2_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  unsigned tree_sitter_html2_external_scanner_serialize(void *payload, char *buffer)
  {
    Scanner *scanner = static_cast<Scanner *>(payload);
    return scanner->serialize(buffer);
  }

  void tree_sitter_html2_external_scanner_deserialize(void *payload, const char *buffer, unsigned length)
  {
    Scanner *scanner = static_cast<Scanner *>(payload);
    scanner->deserialize(buffer, length);
  }

  void tree_sitter_html2_external_scanner_destroy(void *payload)
  {
    Scanner *scanner = static_cast<Scanner *>(payload);
    delete scanner;
  }
