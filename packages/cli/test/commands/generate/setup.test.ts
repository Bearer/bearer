...
describe('generate:setup', () => {
  Object.keys(testCases).forEach(authType => {
    describe(authType, () => {
      const auth = testCases[authType]

      it('creates setup files', async () => {
        const result: string[] = []
        jest.spyOn(process.stdout, 'write').mockImplementation(val => {
          result.push(val)
        })
        const bearerPath: string = ensureBearerStructure({ authConfig: auth, folderName: authType })
        ...
        // TODO: sort out why it fails when running `yarn test`
        // expect(result.join()).toContain('Generating setup components [completed]')
        // expect(result.join()).toContain('Generating setup intents [completed]')
